...
class PBTreeReader(object):
    ...
    def itemsiter(self, prefix):
        ...
        for key, value in itertools.dropwhile(lambda k_v: not k_v[0].startswith(prefix), self.dataiter(first_block)):
...
class IndexBlockReader(object):
    ...
    def __iter__(self):
        ...
        while not end_of_block:
            ...
            # near the end of the block, eveything from here on should be null
            # bytes
            if key == '':
